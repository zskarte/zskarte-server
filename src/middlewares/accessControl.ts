...
export default <T extends Common.UID.ContentType>(config: AccessControlConfig<T>, { strapi }: { strapi: Strapi }) => {
  const logAccessViolation = (ctx, message: string, userOrganisationId, jwtOperationId) => {
    strapi.log.warn(`[global::accessControl]: ${message}, url:${ctx.request.url}, userOrganisationId:${userOrganisationId}, jwtOperationId:${jwtOperationId}, ip:${ctx.request.ip}, user-agent:${ctx.request.headers['user-agent']}`);
  }
  const doCreateChecks = async (ctx, next, userOrganisationId, jwtOperationId) => {
    ...
    if (ctx.request.body.data?.id !== undefined){
      ...
      logAccessViolation(ctx, `create with forcing entry id, ctx.request.body.data?.id:${JSON.stringify(ctx.request.body.data?.id)}`, userOrganisationId, jwtOperationId);
    ...
    if (hasOperation(config.type)){
      if (!ctx.request.body.data?.operation){
        ...
        logAccessViolation(ctx, `create with no operation, ctx.request.body.data?.operation:${JSON.stringify(ctx.request.body.data?.operation)}`, userOrganisationId, jwtOperationId);
        return ctx.forbidden('This action is forbidden.');
      }
      if (jwtOperationId) {
        if (ctx.request.body.data?.operation !== jwtOperationId){
          logAccessViolation(ctx, `create with other operation, ctx.request.body.data?.operation:${JSON.stringify(ctx.request.body.data?.operation)}`, userOrganisationId, jwtOperationId);
          return ctx.forbidden('This action is forbidden.');
        }
      } else {
        let operation = null;
    ...
    if (hasOrganization(config.type)){
      if (!ctx.request.body.data?.organization || ctx.request.body.data?.organization !== userOrganisationId) {
        logAccessViolation(ctx, `create with other/no organization, ctx.request.body.data?.organization:${JSON.stringify(ctx.request.body.data?.organization)}`, userOrganisationId, jwtOperationId);
        return ctx.forbidden('This action is forbidden.');
      }
    }
    return next();
  ...
  const doByIdChecks = async (ctx, next, userOrganisationId, jwtOperationId) => {
    ...
    if (jwtOperationId && operation && jwtOperationId === operation.id) {
      ...
      logAccessViolation(ctx, `access not allowed, entry:${JSON.stringify(entry)}, paramId:${paramId}, headerOperationId:${headerOperationId},`, userOrganisationId, jwtOperationId);
      return ctx.forbidden('This action is forbidden.');
    }

    if(config.check === AccessControlTypes.UPDATE_BY_ID) {
      return doUpdateCheck(ctx, next, userOrganisationId, jwtOperationId, entry, operation, organization, entryId, paramId, headerOperationId)
    }
    return next();
  }
